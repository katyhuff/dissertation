...
where the mass $m_{1i}$ in $[kg]$ of a radionuclide $i$ relased into the waste package
...
%almost no corrosion aqueous oxidizing conditions, which means a steel corrosion 
...
When the waste package is breached and nuclides are released from the waste 
...
\subsubsection{Hydrogeologic Transport Models}
...
volume,  solute transport in a permeable medium of homogenous porosity can be
...
For uniderectional flow, the unidirectional dispersion tensor gives 
...
abstractions based on this solution will be used to advance the concentration 
...
difference and the sum of thermal resistances of the multiple lumps, $R_i 
[W\cdot K^{-1}]$
...
lumped capacitance solver. Originally created for optimal waste loadin analysis 
of the \gls{YMR}, the Bauer model is geometrically adjustable in two dimesions,   
...
spatial waste loading in order to meet them with maximal waste loading. For 
example, given a constraint at the edge of the waste package, the model 
utilizes the \gls{SINDAG} lumped capacitance solver to 
determine the two dimensional heat evolution of the repository 
as a result of a given waste package composition for various drift spacings and 
arrives at an ideal drift spacing by iteration.
...
integrals.  They are listed in Table \ref{tab:heat}.
