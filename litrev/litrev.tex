...
on water flow, nuclide solubility, and nuclide diffusion, but is i
...
\subsubsection{Hedin Model (\cite{hedin_radionuclide_2002})}
In the Hedin model of the Swedish KBS repository model, waste matrix
dissolution is taken to occur at a steady rate. That is, degradation
and congruent nuclide release occur at a constant rate until the the 
fuel matrix is wholly degraded. Gradual dissolution of the initial 
mass $M_{oi}$ of a nulcide $i$ can be described by a production rate 
$P(t)$,
\begin{align*}
...
In the Ahn models, nuclides with a high solubility coefficient are modeled with the 
congruent release model.  Nuclides of this type include most of the 
fission products, but not the actinides. This model states that the 
release from the waste packages is congruent with the dissolution of 
the waste matrix and is transported through the rock by advective 
transfer with the water that flows through the waste packages.  

\subsubsection{Solubility Limited Release Model}

In the Ahn models, nuclides with lower solubility coefficients are modeled with the 
...
\paragraph{Advective transfer rate} 
...
% TSPA for wipp?
% Sweden?
% France?  Figure out which repositories acutally exist in the world.  
% There's certainly a repository for each. 

\subsection{Models Incorporated into Systems Analysis Codes}


%%%% FIX THIS TO AVOID PLAIGARIZING!! %%
To date, most of these top-level simulators ignore the waste disposal 
...
\subsubsection{NUWASTE} Nuclear Waste Technical Review Board code that 
