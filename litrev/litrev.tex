...
\subsection{VISION} \gls{VISION}'s repository model conducts decay calculations
...
The lumped heat capacitance model makes an analogy to electrical circuits. Heat  
flux is analgous to current, resistance analgous to heat conductivity 
connections ( ? ), etc. Components within the heat transfer circuit are
represented by  characteristic lumped parameters which take the place of 
resistors, capacitors, etc.


\subsection{Conduction}

When convection can be neglected, a pure conduction model can be used to 
represent heat transfer.  The dimensionless number which determines the 
appropriateness of neglecting convection is the (<++>) number.

\subsection{Convection}

When conduction can be neglected, a pure convection model can be used to 
represent heat transfer.  The dimensionless number which determines the 
appropriateness of neglecting convection is the (<++>) number.
...
a MathCAD solution, with explicit error function evaluation, of the transient 
conduction equation. The solution of this equation is then treated as a boundary  
condition to the steady state solver, which advances the simulation and 
calculates values through the geometry. The process is then iterated with  
