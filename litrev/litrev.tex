...
simulator and recent efforts on the NUWASTE simulator have made
...
be spatially homogeneous \cite{radel_repository_2007}
...
capacity metrics or conduct detailed nuclide or heat transport.  
...
\glspl{GDSM} for various geological environments. Teams from Argonne
...
project includes an overarching generic model which incorporates the various 
...
off the shelf simulation environment which can include a contaminant transport 
...
Granite disposal concepts have been considered in Finland, Japan, Sweden and 
Switzerland. 

Attributes of granite which make it an attractive candidate geology for nuclear 
...
United States. Attributes of this concept which are favorable for waste 
...
a reversible process , which means the contaminant might be returned to the 
fracture from the matrix with the same ''distribution coefficient`` with which 
they entered the rock matrix \cite{ahn_mass_1988} . 
...
array of degradation, alteration, and dissolution phenomena. 
...
Clay, shale, granite, and salt can largely be characterized as permeable porous media. 
...
\begin{align*}
  C(\vec{r},t) = C_0(\vec{r},t)\mbox{for} \vec{r} \in \Gamma
\end{align*}
...
\begin{align*}
  \frac{\partial C(\vec{r},t)}{\partial x} = nD\vec{J} \mbox{for} \vec{r} \in \Gamma
\end{align*}
...
\cite{anderson_applied_1992} \cite{schwartz_fundamentals_2004}.
...
require an analysis which determines the appropriate length scale for the lumped 
