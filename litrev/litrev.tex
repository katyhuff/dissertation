...
\subsection{Lumped Capacity Model}
...
parameter model may be expected to give a result within $5\%$ error. 
\textbf{CHECK THIS NUMBER, FIND SOURCE, EXCEPTIONS.}
...
\begin{align*}
  ...
  G_{conv} &= h\cdot A\\
  G_{mf}  &= \dot{m}c_p\\
  G_{rad}  &= G_{in}\left[ T_i + T_A + T_j + T_A 
