...
The source term boundary condition is all mass in the available degraded fluid,
\begin{align}
...
\begin{align}
\mathcal{N}_j(t_n)& =-D\frac{dC(t_n)}{dr}\Bigg|_{r=r_j}\nonumber\\ 
                  &= -D\frac{C_k(r_{k-1/2},t_{n-1}) - C_i(r_{j-1/2}, t_n)}{r_{k-1/2} - r_{j-1/2}}
\label{neumann_mixed}
\intertext{where}
