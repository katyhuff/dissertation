...
\begin{align}
C_{df} &= \frac{m_{df}}{V_{df}}\nonumber\\
C_{ds} &= \frac{m_{ds}}{V_{ds}}\nonumber\\
C_{if} &= \frac{m_{if}}{V_{if}}\nonumber
\intertext{and}
C_{is} &= \frac{m_{is}}{V_{is}}.\nonumber
\label{c_m_notation}
\end{align}
...
\begin{align}
C_{ffT}(t_n) &= \left[C_0 + \frac{\int_{t_0}^{t_n} \dot{m}_{i}(t') dt'}{V_{df}(t_n)}\right] 
\intertext{and}
m_{ffT}(t_n) &= C_{ff}(t_n) V_{df}(t_n)\nonumber\\
       &= \left[C_0 + \frac{\int_{t_0}^{t_n} \dot{m}_{i}(t') dt'}{V_{df}(t_n)}\right] V_{df}(t_n) \nonumber\\
       &= C_0V_{df}(t_n) + \int_{t_0}^{t_n} \dot{m}_{i} dt'.
\end{align}
...
m_{psc} &= s_{ps}m_{psT}\nonumber\\
          &= K_dC_{ffl}m_{psT}\nonumber\\
          &= \frac{K_dm_{ffl}m_{psT}}{V_{df}}\nonumber\\
          &= \frac{K_d}{V_{df}}(m_{ffT}-m_{psc})m_{psT}\nonumber\\
          &= \frac{K_d}{V_{df}}(m_{ffT}-m_{psc})(m_{psm}+m_{psc})\nonumber\\
          &= \frac{K_d}{V_{df}}(m_{ffT}m_{psm}-m_{psc}m_{psm} + m_{ff}m_{psc} -m_{psc}^2)\nonumber\\
          &= \frac{K_d}{V_{df}} (m_{ffT}m_{psm} + (m_{ffT} - m_{psm})m_{psc} - m_{psc}^2)\nonumber
\intertext{which, rearranged, becomes }
0         &= m_{psc}^2 + \left( -m_{ffT} + m_{psm} + \frac{V_{df}}{K_d} \right)m_{psc} - m_{ffT}m_{psm}\nonumber
\intertext{and is solved using the quadratic formula, such that}
m_{psc}   &= \frac{m_{ffT} - m_{psm} - \frac{V_{df}}{K_d}}{2}
             \pm\frac{\sqrt{\left(-m_{ffT} + m_{psm} + \frac{V_{df}}{K_d}\right)^2 -4m_{ffT}m_{psm}}}{2}\nonumber\\  
          %&= \frac{m_{ffT} - m_{psm} - \frac{V_{df}}{K_d}}{2} \nonumber\\
          %& \pm\frac{\sqrt{m_{ffT}^2 + m_{psm}^2 + \frac{V_{df}^2}{K_d^2} + 2m_{ffT}m_{psm} - 
          %   \frac{2V_{df}m_{ffT}}{K_d} + \frac{2V_{df}m_{psm}}{K_d} } }{2} \nonumber
\intertext{which, again rearranged, becomes}
          &= \frac{1}{2} \left(m_{ffT} - m_{psm} - \frac{V_{df}}{K_d}\right) \nonumber\\
          & \pm \frac{1}{2} \sqrt{m_{ffT}^2 + 2m_{ffT}\left(m_{psm} - 
          \frac{V_{df}}{K_d}\right) + \left(m_{psm} + 
          \frac{V_{df}}{K_d}\right)^2}.
\label{m_psc}
\end{align}
...
\begin{align}
m_{ffl}   &= m_{ffT} - \frac{1}{2} \left(m_{ffT} - m_{psm} - \frac{V_{df}}{K_d}\right) \nonumber\\
...
\begin{align}
C(x,y,z,t) = \frac{m_{ffl}(t)}{V_{df}(t)}\forall (x,y,z) \in \Gamma.
