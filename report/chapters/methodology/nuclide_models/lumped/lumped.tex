...
m_j(t_n) &= \int_0^V \theta C(r) dV \nonumber\\
         ...
         &= 2\pi h\theta \int_{r_i}^{r_j}\left( \frac{C_j(t_n) - C_i(t_n)}{r_j - r_i}r^2 + C_i(t_n)r \right) dr\nonumber
\intertext{such that}
m_j(t_n) &= 2\pi h \theta \left[ \frac{C_j(t_n) - C_i(t_n)}{3\left(r_j - r_i\right)}(r_j-r_i)^3 + C_i(t_n) \frac{(r_j-r_i)^2}{2}\right] 
\end{align}
