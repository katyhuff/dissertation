...
For a primarily advective interface between two components, the user may choose 
the advective flux mass transfer mode. The advective flux across 
the boundary between two components, 

\begin{align}
J_{adv,j}(t_n) &= \mbox{ potential advective flux from component j at }t_n [kg/m^2/s]\nonumber\\
               &= \theta_k v C(z,t_n)|_{z=r_j}
\end{align}
relies on the fixed concentration Dirichlet 
boundary condition at the interface. That boundary condition is provided by the 
internal component's mass balance model, based on the concentration profile 
...
For primarily dispersive flow across the boundary, the user may choose instead 
to represent mass transfer as purely dispersive between the two components. The 
dispersive flux in one dimension, 
\begin{align}
      J_{dis} &= \mbox{ Total Dispersive Mass Flux }[kg/m^2/s]\nonumber\\
      &= -\theta D\frac{\partial C}{\partial z}\Big|_{z=r_j} \nonumber
\end{align}
relies on the fixed concentration gradient Neumann boundary condition, 
...
\begin{align} 
\frac{dC(z,t_n)}{dz}\Bigg|_{z=r_j} &= \frac{C_{out} - C_{in}}{r_{j} - r_{i}}.
\end{align}
...
Coupled advective and dispersive flow is the sum of these two fluxes, and 
relies on both boundary conditions.
