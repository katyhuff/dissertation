\subsection{Degradation Rate Radionuclide Mass Balance Model}\label{sec:deg_rate}
The degradation rate model is the simplest of implemented models and is most 
...
mechanisms responsible for this degradation, but instead generically captures 
...
steps are assumed to be small enough to assume a constant rate $m_{ij}$ over 
...
In terms of a constant rate, the volume degraded between discrete times $t_{n-1}$ and 
$t_n$ is thus a simple linear function of the transfer rate,
...
are available to adjacent components. The total contaminants $m_{ij}(t)$, 
available for mass transfer from cell $i$ to cell $j$ at time $t$ are thus 
...
During a timestep, this model incorporates the mass flux from the
inner boundary into its available mass and recalculates the mass balance.
