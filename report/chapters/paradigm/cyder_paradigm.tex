...
\begin{align}
  \dot{m}_{ij}^n &= f( m_j^{n-1}, BC_i^n, BC_j^{n-1}. . . ) \nonumber\\
  \intertext{where}
  m_{ij}^n &= \mbox{ contaminant mass flux from component i to j }[kg/timestep]
...
\begin{align}
  m_i^{n*} &= m_i^n - \dot{m}_{ij}^n
  \intertext{where}
  m_i^{n*} &= \mbox{ updated mass in component i }[kg]
\end{align}
In this way, the contained mass in the component is described as
\begin{align}
  m_j^n &= m_j^{n-1} + \dot{m}_j^n. \nonumber
\end{align}

Resulting concentration profiles accross the component can then be calculated 
and one can solve, numerically, for the outer boundary condition at $t_n$ 

\begin{align}
  BC_j^n &= g\left( m_j^n, C_j^n \right)\nonumber\\
