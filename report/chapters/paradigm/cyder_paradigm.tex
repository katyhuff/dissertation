...
That is, in Component j, some Component in a nested series, the mass or concentration 
distribution in the Component at time $t_n$ is found from the initial state of the cell 
at time $t_n$, the inner boundary 
condition at time $t_n$ and the outer boundary condition at $t_{n-1}$. For the 
fundamental example of mass calculation (on which concentration calculations 
rest),

\begin{align}
  m_j^n &= f( m_j^{n-1}, BC_i^n, BC_j^{n-1}. . . ) \nonumber
  \intertext{where}
  m_j^n &= \mbox{ the contaminant mass vector in cell j }[kg]
  BC_i^n &= \mbox{ inner conditions at $r_i$, and time $t_n$ }\nonumber \\
  BC_j^n &= \mbox{ outer conditions at $r_j$, and time $t_{n-1}$ }\nonumber\\
...
the outer boundary condition at $t_n$ 

\begin{align}
  BC_j^n &= g\left( m_j^n \right)\number\\
  g &= \mbox{functional form of contaminant transport accross j}\nonumber
\end{align}

which can, in turn, be used by the component external to it, $k$ as the $t_n$ 
