...
geologic host media under consideration domestically and internationally are then also 
...
transport on long time scales and in various geologic media. 
...
\subsubsection{Repository Performance Calculations}
...
Enclosed concepts are appropriate for low permeability rock formations 
(clay/shale, granite, salt).  Low permeability does not permit significant 
oxygen entry in an enclosed concept, these are typically chemically reducing 
environments. 
...
has occurred, heat limits are often lower in enclosed repository concepts than in open 
...
Attractive qualities of clay for repository investigations include its ease of 
excavation as well as the tendency, particularly for some very plastic clays 
under consideration, to coalesce over time around waste packages within the 
zone disturbed by excavation. Additionally, clays tend to have high sorption 
...
\begin{align}
  V_{WF}(t) = V_{WF}(0) - \int_0^t f(\cdots)dt.
  \label{wfdegrate}
\end{align}
...
\begin{align}
  V_{WF}(t) &= V_{WF}(0) - \int_0^t f(\cdots)H(t-t_{wpf})dt.
  \intertext{where}
...
flowthrough water at a fractional degradation rate until the waste form is 
completely degraded \cite{ahn_environmental_2004, ahn_environmental_2007}.
...
\begin{align} 
  m_{1i}(t)&\le V_{1i}(t)C_{sol,i}
  \intertext{where}
  m_{i1} &= \mbox{ mass of isotope i in volume }V_1 [kg]\nonumber\\
  V_{1} &= \mbox{ a distinct volume of fluid } [m^3]\nonumber\\
  C_{sol} &= \mbox{ the maximum concentration for the element of i } [kg\cdot m^{-3}].\nonumber
\end{align}
...
Water flowthrough heavily affects the waste 
...
In common flowthrough models, the water flow begins at 
...
If it can be estimated, a fractional degradation rate can be used for waste 
package degradation as it was in the waste form case,

\begin{align}
  V_{wp}(t) &= V_{wp}(0)(1 - \int_0^t f(\cdots)dt)
  \intertext{which, for a constant rate, becomes}
  V_{wp}(t) &= V_{wp}(0)(1 - f(\cdots)t).
  \label{wfdegrate}
\end{align}
...
Advective transfer through the engineered barrier system and into the geologic medium 
...
\begin{align}
  C(\vec{r},t) = C_0(\vec{r},t)\hspace{1mm}\mbox{ for } \vec{r} \in \Gamma\\
  \frac{\partial C(\vec{r},t)}{\partial r} = \theta D\vec{J} \hspace{1mm}\mbox{ for } \vec{r} \in \Gamma.
\end{align}
...
Detailed computational models of radionuclide transport seek to quantify the 
spatial movement of radionuclides within a repository. Such models are detailed 
with respect to their fine spatial and temporal resolution or with respect to 
the incorporation and coupling of numerous physical phenomena. Current detailed 
...
The \gls{UFD} campaign has produced a \acrlong{GPAM} for analysis of various 
