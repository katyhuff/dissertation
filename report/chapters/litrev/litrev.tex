...
computational models of radionuclide and thermal transport follows. 
...
\gls{COSI} \cite{boucher_international_2010}) utilize discrete material tracking to 
...
tracking, the power of dynamic modeling, and the flexibility of open access the \Cyder 
...
models, some repository focused fuel cycle sensitivity analyses have been 
conducted using existing tools. 
...
and others. With a focus on \gls{YMR} capacity, 
...
steps through time in 8 hour time steps during the waste cask transportation, 
...
reactions in these environments proceed in a reducing direction. This attribute 
has the primary effect of slowing corrosion, dissolution, and  alteration rates 
in materials that are subject to degradation. A reducing disposal environment 
also lowers solubility limits and increases sorption of many actinide species. 
The dominant dose contributors in reducing environments are therefore the 
soluble, long lived fission and activation products such as $^{129}I$ and 
$^{79}Se$ 
...
both domestic and international repository concepts, models were developed 
...
potential long-term dose to the public from clay repository concepts.  For most fuel 
...
Well understood behavior for argillite and bentonite buffer backfill
...
%as with the waste packages in this geologic medium.  
...
used values between 1.25 and 1.7 W/m$\cdot$K,
France (ref. \cite{andra_argile:_2005}) used values between 1.9 and 2.7 W/m$\cdot$K, and Switzerland (ref. 
\cite{johnson_calculations_2002}) used 1.8 W/m$\cdot$K.
...
The Spanish concept is almost identical to 
...
2.4 and 4 W/m$\cdot$K.  In particular Sweden (ref. 
\cite{ab_long-term_2006}) used 3.4 - 4 W/m$\cdot$K and 2.45 - 2.9 W/m$\cdot$K,
France (ref.  \cite{andra_argile:_2005}) 
used 2.4 - 3.8 W/m$\cdot$K, and Finland (ref.
\cite{posiva_interim_2010}) used 2.3 - 3.2 W/m$\cdot$K.
...
far field near the earth's surface. The very high expected salinity of this 
environment, due to its depth, indicates historically low flow. While this increases 
...
environment is identical to the granite case, except the bentonite buffer 
...
\begin{align}
  ...
  \intertext{where}
  f(\cdots) &= \mbox{ a known degradation rate function }
  \label{wfdegrate}
\end{align}
...
contact. Either environment can be represented by the general expression, 
...
If congruent release neglects transport limiting factors such as solubility 
...
In order to account for solubility limitation, an upper limit of elemental 
...
In the Ahn models \cite{ahn_environmental_2004, ahn_environmental_2007}, radionuclides with lower solubility limits are modeled with a solubility limited release model.  Solubility values are assumed from TSPA for this model, and elements with a solubility of less than $~5\times 10^{-2} $ mol/m$^3$ are taken to be ``low.'' Elements in this ``low'' category include Zr, Nb, Sn and some toxic actinides such as Th and Ra for an oxidizing, unsaturated environment similar to \gls{YMR}.
...
gradient between the waste matrix and the water. 

\subsubsection{Flow Assumptions}

Water heavily affects the waste form degradation rate and is treated differently 
in various models. Some ``flow through'' models assume water moves through 
...
incorporating climate-based predictions of hydrologic activity 
...
limited by a release resistance coefficient \cite{hedin_integrated_2002}. The release is assumed  to occur
...
interface \cite{hedin_integrated_2002}. A special case of this is instantaneous 
waste package failure, in which all waste  packages simultaneously such that
...
Otherwise, the analytical expression in equation \eqref{solperm} was the 
...
\begin{align}
  ...
  \label{withsorption} 
  \intertext{where}
  s &= \mbox{solid concentration }[kg/kg]\nonumber\\
...
s &= K_dC\\
\intertext{where}
K_d &= \mbox{ the distribution coefficient }[m^3/kh],\nonumber
\end{align}
...
diffusive fluxes at  the boundary of the domain
...
The third, Cauchy, type defines a linear combination of the concentration 
gradient and a concentration at that boundary, 

\begin{align}
  -D\frac{\partial C(\vec{r}, t)}{\partial r}\Bigg|_{r\in \Gamma} + 
  v_zC_j\Bigg|_{r\in\Gamma} = \mbox{a known value }[kg/m^2s]
\end{align}
...
system and host rock. This model must sufficiently solve for peak temperatures 
...
host rock. These heat limits were discussed in Section 
\ref{sec:claythermal}. 
...
For the one dimensional case, equation \ref{fourier} can be approximated using a 
finite difference method. For a body at $x_1$ with temperature $T_1$
...
radionuclide transport within its respective geologic medium 
\cite{clayton_generic_2011}. 
...
The Clay \gls{GDSM} is being pursued by the team at \gls{ANL} and was 
the primary model with which this work conducted parametric regression 
analyses. 
...
porosities of contained materials (e.g. crushed salt). A block  of rock 
...
\subsection{Li Model \cite{li_methodology_2006}} In a case study evaluating 
transmutation effects on repository benefit, Jun Li approximated Yucca Mountain 
Repository performance for three fuel cycles. As a function of time, water
...
Waste package failure and radionuclide release are modeled with two 
\gls{TSPA}
...
\subsection{ANDRA Dossier 2005} The ANDRA Dossier 2005 studies, developed for 
the French radioactive waste disposal program, provided
detailed radionuclide transport calculations for both argillite, a clay-rich 
...
assumes that the excavation disturbed zone does not heal. Rather, it is modeled 
...
model or a two phase model. In the first phase dissolution is treated by a rate model 
until silica saturation in the surrounding environment. In the second phase, 
dissolution kinetics decreases to a residual rate.

%(pg198 for references to themath)

Transport through the backfill is modeled as diffusive, with high permeability 
after degradation.  The excavation disturbed zone has both a fracture zone and a 
...
detailed with respect to their spatial and temporal resolution or 
...
\begin{figure}S]
  ...
  \end{center}
  \caption[A schematic of the SINDAG geometry.]{The geometry of the 
...
With these representations of thermal resistance, a lumped parameter model 
required an analysis that determines the appropriate length scale for the lumped 
